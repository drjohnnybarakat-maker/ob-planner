[index.html](https://github.com/user-attachments/files/27698866/index.html)
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
...
<link rel="apple-touch-icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><rect width='100' height='100' rx='22' fill='%230e0a08'/><text y='.9em' font-size='80' x='10'>🌸</text></svg>"/>
...
/* ═══════════════════════════════════════
   SCREEN 1 — FORM
═══════════════════════════════════════ */
.form-scroll { padding: 20px 16px; padding-bottom: calc(110px + env(safe-area-inset-bottom, 34px)); }
...
<!-- FLOATING GENERATE BUTTON — fixed, always on screen -->
<div id="generateBtnWrap" style="
  position: fixed;
  bottom: 0; left: 0; right: 0;
  padding: 14px 16px;
  padding-bottom: calc(14px + env(safe-area-inset-bottom, 24px));
  background: linear-gradient(to top, #0e0a08 60%, transparent);
  z-index: 50;
">
  <button class="generate-btn" onclick="generate()" style="
    width: 100%; padding: 18px; font-size: 17px;
    box-shadow: 0 8px 40px rgba(232,135,106,0.45);
  ">
...
function switchTab(i) {
  ...
  // Show generate button only on patient tab, and only if not yet generated
  const btn = document.getElementById('generateBtnWrap');
  if (btn) btn.style.display = (i === 0) ? 'block' : 'none';
}
...
function buildSummary() {
  ...
  document.getElementById('summaryContent').innerHTML = `
    ...
    <div class="action-row animate-in delay-2" style="margin-top:-4px;">
      <button class="action-btn" style="background:rgba(180,140,255,0.12);border:1.5px solid rgba(180,140,255,0.3);color:#c4a0ff;" onclick="sharePDF()">
        <span class="btn-ico">📤</span>
        Share File (AirDrop / iMessage / WhatsApp)
      </button>
    </div>
  `;
}
...
function buildSchedule() {
  ...
  container.innerHTML = `
    <div class="action-row" style="margin-bottom:8px;">
      <button class="action-btn btn-email" onclick="openEmailSheet()"><span class="btn-ico">✉️</span>Outlook</button>
      <button class="action-btn btn-wa" onclick="openWaSheet()"><span class="btn-ico">📱</span>WhatsApp</button>
      <button class="action-btn btn-print" onclick="openPrintSheet()"><span class="btn-ico">🖨️</span>SharpDesk</button>
    </div>
    <div class="action-row" style="margin-bottom:12px;">
      <button class="action-btn" style="background:rgba(180,140,255,0.12);border:1.5px solid rgba(180,140,255,0.3);color:#c4a0ff;" onclick="sharePDF()">
        <span class="btn-ico">📤</span>Share File (AirDrop / iMessage / WhatsApp)
      </button>
    </div>
  `;
...
function openEmailSheet() {
  ...
  // ms-outlook:// is the ONLY link used — no mailto fallback so Mail app never opens
  const outlookHref = `ms-outlook://compose?to=${encodeURIComponent(email||'')}&subject=${subj}&body=${body}`;
  document.getElementById('emailSheetLink').textContent = email || '(no email entered)';
...
function openWaSheet() {
  ...
  document.getElementById('waSheetLink').textContent = phone ? `wa.me/${phone}` : 'No number entered';
  document.getElementById('waSheetBtn').onclick = function(e) {
    e.preventDefault();
    closeSheet('waSheet');
...
function generateAndSharePDF() {
  ...
  setTimeout(() => {
    try {
      ...
      // On iOS, downloading the blob as a file gives a .html file
      // that opens in Files app, from where user picks SharpDesk Mobile
      const a = document.createElement('a');
      a.href = blobUrl;
      const filename = `Prenatal_${state.firstName}_${state.lastName}.html`.replace(/\s+/g,'_');
      a.download = filename;
...
// ─────────────────────────────────────
// PDF SHARE (AirDrop / WhatsApp / iMessage)
// ─────────────────────────────────────
function sharePDF() {
  if (!state.generated) { showError('Please generate a schedule first.'); return; }

  // Build print HTML and download as file
  // iOS: after download, tap the file → Share ⬆ → choose AirDrop / WhatsApp / iMessage
  const html = buildPrintHTML();
  const blob = new Blob([html], { type: 'text/html' });
  const blobUrl = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = blobUrl;
  a.download = `Schedule_${state.firstName}_${state.lastName}.html`.replace(/\s+/g,'_');
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  setTimeout(() => URL.revokeObjectURL(blobUrl), 3000);
...
<div class="sheet-overlay" id="printSheet" onclick="closeSheet('printSheet')">
  <div class="bottom-sheet" onclick="event.stopPropagation()">
    ...
    <div class="sheet-sub" id="printSheetSub">Downloads the schedule as a file. When it appears in your Downloads, tap it → tap <strong style="color:var(--gold)">Share ⬆</strong> → choose <strong style="color:var(--gold)">SharpDesk Mobile</strong> to send to your Sharp printer.</div>
...
<!-- BOTTOM SHEET: SHARE FILE -->
<div class="sheet-overlay" id="shareFileSheet" onclick="closeSheet('shareFileSheet')">
  <div class="bottom-sheet" onclick="event.stopPropagation()">
    <div class="sheet-handle"></div>
    <div class="sheet-title">📤 Share Schedule File</div>
    <div class="sheet-sub">The file is downloading now. To share it via AirDrop, iMessage or WhatsApp:</div>
    <div style="background:var(--surface2);border-radius:14px;padding:14px 16px;margin-bottom:14px;">
      <div style="font-size:13px;color:var(--text);line-height:2.0;">
        1. Tap the <strong style="color:var(--gold);">⬇ Downloads</strong> icon in Safari's top bar<br/>
        2. Tap the <strong style="color:var(--text);">Schedule_...</strong> file<br/>
        3. Tap <strong style="color:var(--rose-light);">Share ⬆</strong><br/>
        4. Choose <strong style="color:#c4a0ff;">AirDrop</strong>, <strong style="color:#25d366;">WhatsApp</strong>, or <strong style="color:#5ac8fa;">iMessage</strong>
      </div>
    </div>
    <button class="sheet-btn sheet-btn-secondary" onclick="closeSheet('shareFileSheet')" style="font-family:inherit;">Got it</button>
  </div>
</div>
